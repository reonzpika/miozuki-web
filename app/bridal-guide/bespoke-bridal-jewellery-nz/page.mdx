...
{/* VERIFY: Confirm the design and delivery process: what the initial conversation covers and how it's conducted, what format the rendering/sketch/3D model takes and when you receive it, and whether remote collection or shipping is offered for customers outside Auckland. */}
...
{/* VERIFY: the journey diagram above is AI-generated as a flat icon infographic (2026-07-09 diagram set), confirm you are happy with its look and that the five steps match your actual process before launch */}
...
{/* VERIFY: Confirm the full timeline and process flow: when stone sourcing happens relative to metalworking and setting, the typical number of weeks from design approval to completion, what progress updates are offered and how often, and the order of the steps in between. */}
...
{/* VERIFY: Confirm studio location (full street address and hours) and whether remote try-on appointments or shipping is offered for customers outside Auckland. */}
...
{/* VERIFY: Confirm whether Miozuki's typical bespoke bridal rings exceed the AUD $1,000 customs duty threshold, the exact duty percentage applicable, any customs broker handling fees, whether Miozuki manages the customs paperwork and shipping or whether the customer arranges this independently, and the typical transit time from Miozuki to Australian delivery. */}
...
{/* VERIFY: Confirm warranty terms: what's covered under the guarantee (craftsmanship, materials, stones), how long the warranty lasts, and what Miozuki's policy is on repairs or remake if the ring needs work after purchase. */}
